`ifndef SETTINGS_VH
`define SETTINGS_VH

// `define testbench "C:\\Users\\19438\\Documents\\GitHub\\PipelineCPU\\testbench\\multi-naive.txt"
`define testbench "C:\\Users\\lenovo\\Desktop\\计算机系统实现I\\Pipeline\\Mips50TestCodeAns\\Mips50TestCodeAns\\TestCode\\HexadecimalCode\\88H.asm.txt"
`define outputPath "D:\\CSI\\pipeline\\ans\\88H.asm.txt"
// output "D:\CSI\pipeline\ans\88H.asm.txt"
// ans "C:\Users\lenovo\Desktop\计算机系统实现I\Pipeline\Mips50TestCodeAns\Mips50TestCodeAns\TestAns\88H.asm.txt"
//`define DEBUG
// `define SHOW_STALLS

typedef struct packed 
{
    logic reset;
    logic clock;
} SystemSignal;

typedef logic [31:0] Vec32;
typedef logic [5:0] Vec6;
typedef logic [4:0] Vec5;
typedef logic [1:0] Vec2;
typedef logic [2:0] Vec3;
typedef logic [32:0] Vec33;
typedef logic [25:0] Vec26;
typedef logic [15:0] Vec16;

typedef enum Vec6
{
    ADD = 6'b100000,
    UADD = 6'b100001,
    SUB = 6'b100010,
    USUB = 6'b100011,
    LSHIFT = 6'b000000,
    LRSHIFT = 6'b000010,
    ARSHIFT = 6'b000011,
    LSHIFTV = 6'b000100,
    LRSHIFTV = 6'b000110,
    ARSHIFTV = 6'b000111,
    AND = 6'b100100,
    OR = 6'b100101,
    XOR = 6'b100110,
    NOR = 6'b100111,
    SLT = 6'b101010,
    SLTU = 6'b101011
} ALUOp_t;//ALU的Op集合

typedef enum Vec32
{
    // 6 + 5 + 5 + 5 + 5 + 6 = 32
    nop     = 32'b000000_00000_00000_00000_00000_000000,

    // codet + rs + rt + rd + shamt + funct
    addu    = 32'b000000_?????_?????_?????_00000_100001,
    subu    = 32'b000000_?????_?????_?????_00000_100011,
    add     = 32'b000000_?????_?????_?????_00000_100000,
    sub     = 32'b000000_?????_?????_?????_00000_100010,
    sll     = 32'b000000_00000_?????_?????_?????_000000,
    srl     = 32'b000000_00000_?????_?????_?????_000010,
    sra     = 32'b000000_00000_?????_?????_?????_000011,
    sllv    = 32'b000000_?????_?????_?????_00000_000100,
    srlv    = 32'b000000_?????_?????_?????_00000_000110,
    srav    = 32'b000000_?????_?????_?????_00000_000111,
    And     = 32'b000000_?????_?????_?????_00000_100100,
    Or      = 32'b000000_?????_?????_?????_00000_100101,
    Xor     = 32'b000000_?????_?????_?????_00000_100110,
    Nor     = 32'b000000_?????_?????_?????_00000_100111,
    slt     = 32'b000000_?????_?????_?????_00000_101010,
    sltu    = 32'b000000_?????_?????_?????_00000_101011,
    jr      = 32'b000000_?????_00000_00000_?????_001000, // what is hint?
    jalr    = 32'b000000_?????_00000_?????_?????_001001, 
    syscall = 32'b000000_00000_00000_00000_00000_001100, // what is code?
    mult    = 32'b000000_?????_?????_00000_00000_011000,
    multu   = 32'b000000_?????_?????_00000_00000_011001,
    div     = 32'b000000_?????_?????_00000_00000_011010,
    divu    = 32'b000000_?????_?????_00000_00000_011011,
    mfhi    = 32'b000000_00000_00000_?????_00000_010000,
    mflo    = 32'b000000_00000_00000_?????_00000_010010,
    mthi    = 32'b000000_?????_00000_00000_00000_010001,
    mtlo    = 32'b000000_?????_00000_00000_00000_010011,

    // codet + rs + rt + 16'imm
    addi    = 32'b001000_?????_?????_????????????????,
    addiu   = 32'b001001_?????_?????_????????????????,
    andi    = 32'b001100_?????_?????_????????????????,
    ori     = 32'b001101_?????_?????_????????????????,
    xori    = 32'b001110_?????_?????_????????????????,
    slti    = 32'b001010_?????_?????_????????????????,
    sltiu   = 32'b001011_?????_?????_????????????????,
    lw      = 32'b100011_?????_?????_????????????????,
    lb      = 32'b100000_?????_?????_????????????????,
    lbu     = 32'b100100_?????_?????_????????????????,
    lh      = 32'b100001_?????_?????_????????????????,
    lhu     = 32'b100101_?????_?????_????????????????,
    sw      = 32'b101011_?????_?????_????????????????,
    sb      = 32'b101000_?????_?????_????????????????,
    sh      = 32'b101001_?????_?????_????????????????,
    beq     = 32'b000100_?????_?????_????????????????,
    bne     = 32'b000101_?????_?????_????????????????,
    blez    = 32'b000110_?????_00000_????????????????,
    bgtz    = 32'b000111_?????_00000_????????????????,
    bgez    = 32'b000001_?????_00001_????????????????,
    bltz    = 32'b000001_?????_00000_????????????????,
    lui     = 32'b001111_00000_?????_????????????????,

    // codet + 26'imm
    jal     = 32'b000011_??????????????????????????,
    j       = 32'b000010_??????????????????????????
} InstructionCode_t;//Instrution的指令集合

typedef enum Vec2 
{  
    rtDst = 2'b00,
    rdDst = 2'b01,
    raDst = 2'b10,
    otherDst = 2'b11
} RegWriteDst_t;//寄存器写的对象

typedef enum Vec3
{
    rtAluSrc = 3'b000,
    signExtOfImm = 3'b001,
    leftShiftOfImm = 3'b010,
    zeroExtOfImm = 3'b011,
    otherAlu = 3'b100,
    shamtAluSrc = 3'b101
} ALUSrc_t;//ALU的操作数来源

typedef enum Vec2 
{
    regReadA = 2'b00,
    plusImm = 2'b01,
    targetImm = 2'b10,
    normalPC = 2'b11
} PCSrc_t;//PC的选择器

typedef enum Vec3
{  
    alu = 3'b000,
    mem = 3'b001,
    nextPC = 3'b010,
    zeroRegWrite = 3'b011,
    specialReg = 3'b100
} RegWriteSrc_t;//寄存器写的数据来源

// typedef enum Vec2
// {
//     NON_SPECIAL = 2'b00,
//     MULT_SPECIAL = 2'b01,
//     DIV_SPECIAL = 2'b10
// } Special_t;

typedef struct packed {
    InstructionCode_t instructionCode;
    Vec5 rs, rt, rd;
    Vec16 imm16;
    Vec26 imm26;
    Vec6 funct;
    Vec5 shamt;
} Instruction;//指令及其结构拆分组成的结构体

`define reset_Instruction '{    \
    instructionCode: nop,   \
    rs: 5'b00000,   \
    rt: 5'b00000,   \
    rd: 5'b00000,   \
    imm16: 16'b0000000000000000,    \
    imm26: 26'b00000000000000000000000000,  \
    funct: 6'b000000,    \
    shamt: 5'b00000 \
}

typedef struct packed {
    RegWriteDst_t regWriteDst;
    RegWriteSrc_t regWriteSrc;
    ALUSrc_t aluSrc;
    PCSrc_t pcSrc;
    ALUOp_t aluOp;

    logic special;
    logic regWriteEnabled;
    logic memReadEnabled;
    logic memWriteEnabled;
    logic branch;
    logic realBranch;
} ControlSignal;//控制信号集合

`define reset_ControlSignal '{    \
    regWriteDst: otherDst,  \
    regWriteSrc: zeroRegWrite,    \
    aluSrc: otherAlu,   \
    pcSrc: normalPC,    \
    aluOp: UADD, \
    special: 1'b0,   \
    regWriteEnabled: 1'b0,  \
    memReadEnabled: 1'b0,   \
    memWriteEnabled: 1'b0,  \
    branch: 1'b0,   \
    realBranch: 1'b0    \
}//控制信号重置

typedef struct packed {
    int pcValue;
    Instruction instruction;
} IF_ID_Reg;// IF/ID

typedef struct packed {
    int pcValue;
    Instruction instruction;
    ControlSignal signal;
    Vec5 regReadA;
    Vec5 regReadB;
    Vec5 regWrite;
    int regReadDataA;
    int regReadDataB;
} ID_EX_Reg;// ID/EX

typedef struct packed {
    int pcValue;
    Instruction instruction;
    ControlSignal signal;
    Vec5 regWrite;
    int aluResult;
    logic aluOverflow;
    int regReadDataB;
    int specialReg;
} EX_MEM_Reg;// EX/MEM

typedef struct packed {
    int pcValue;
    Instruction instruction;
    ControlSignal signal;
    Vec5 regWrite;
    int aluResult;
    logic aluOverflow;
    int memReadData;
    int specialReg;
} MEM_WB_Reg;// MEM/WB

typedef enum Vec2
{
    Forwarding_GPR = 2'b00, // From Register Files
    Forwarding_EX_MEM = 2'b01, // From Last time ALU Result
    Forwarding_MEM_WB = 2'b10,  // From DM or The Second Last ALU Result
    Forwarding_ID_EX = 2'b11
} Forwarding_t;// 旁路转发

`endif
